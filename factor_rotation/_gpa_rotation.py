...
def GPA(A, dtype, ff=None, vgQ=None, T=None, max_tries=10000,
       rotation_method = 'orthogonal', tol=1e-09):
    
    #pre processing
    tol = tf.cast(tol, dtype=dtype)

    rotation_method = tf.constant(rotation_method)
    method_assert = tf.Assert(  tf.logical_or( tf.equal(rotation_method, "orthogonal"), tf.equal(rotation_method, "oblique" ) ), [rotation_method] )
    with tf.control_dependencies([method_assert]):
        rotation_method = tf.identity(rotation_method)

    p, k = A.get_shape(); p = tf.cast(p, 'int32'); k = tf.cast(k, 'int32')
    if T is None:
    ...
    def outer_body(i_try, Gp, al, s, f, T, Ti, Gq, G, L, ft, Tt):
        ...
        def inner_body(j_try, Gp, al, al_old, s, f, T, ft, Ti, Gq, G, Tt, L):
            ...
            al_old = al
            al = tf.cond( ft < f-.5*s**2*al ,
                lambda: tf.identity(al) ,
                lambda: al / 2)

            return j_try, Gp, al, al_old, s, f, T, ft, Ti, Gq, G, Tt, L

        j_try, Gp, al, al_old, s, f, T, ft, Ti, Gq, G, Tt, L = tf.while_loop(inner_cond, inner_body, [j_try, Gp, al, al_old, s, f, T, ft, Ti, Gq, G, Tt, L])
        # inner_loop
    ...
    i_try, Gp, al, s, f, T, Ti, Gq, G, L, ft, Tt = tf.while_loop(outer_cond, outer_body, [i_try, Gp, al, s, f, T, Ti, Gq, G, L, ft, Tt])
    #outer_loop
...
def rotateA(A, T, rotation_method='orthogonal'):
    L = tf.case( {
        ...
        tf.equal(rotation_method, "oblique"): 
            lambda: tf.matmul( A, tf.matrix_inverse( tf.transpose(T) ))} , 
        default = lambda: tf.matmul(A, T) , exclusive=True )
...
def CF_objective(L=None, A=None, T=None, kappa=0,
    ...
    zero = tf.cast(0, dtype); one = tf.cast(1, dtype);
    cls_zero8 = tf.cast(1e-08, dtype); cls_zero5 = tf.cast(1e-05, dtype)

    if L is None:
        L = rotateA(A, T, rotation_method=rotation_method)
    
    p, k = L.get_shape(); p = tf.cast(p, 'int32'); k = tf.cast(k, 'int32')
    L2 = L**2
...
def oblimin_objective(L=None, A=None, T=None, gamma=0,
    ...
    p, k = L.get_shape(); p = tf.cast(p, 'int32'); k = tf.cast(k, 'int32'); p_fl = tf.cast(p, dtype)
    L2 = L**2
    ...
    X = tf.cond(kap_0 ,
        ...
        lambda: tf.tile( tf.reshape ( (-gamma/p_fl)*tf.reduce_sum(tf.matmul(L2, N), 0), (1, k)), (p, 1)) + tf.matmul(L2, N)
        )
